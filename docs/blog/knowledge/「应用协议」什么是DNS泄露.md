...
在Tun模式下的redir-host模式，不进行远程DnS（指的是由代理节点进行DNS解析），所有的DNS都从本地发起，但使用的却是国外加密DNS服务器，这样获取到的的IP其实是与本机在地理位置相近的CDN服务器，而实际上我们访问外网实际上走的是代理节点，代理节点的地理位置一般都比较远。

也就是说，由于DNS是在国内请求的，DNS请求到的IP地址离国内很近，但是后续访问这个IP地址却是代理节点来访问，这样获取到的就不是距离最段网络最优质的那个线路。

此外，国内直接访问国外的DNS服务器，在速度上又比较慢，带加密的DNS服务器，加密这种功能又会增加延迟。

所以redir-host这种模式下问题比较多，现在基本都不怎么用了，用的比较多的是 fake-ip模式。
